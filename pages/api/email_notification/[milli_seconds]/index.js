import { createEmailNotification, fetcheEmailNotifications, fetcheEmailNotifications_test, updateEmailNotification } from "services/email_notification";
...
export default async function emailNotificationHandler(req, res) {
    ...
    switch (method) {
        ...
        case "GET":
            try {
                let resp = await fetcheEmailNotifications_test(milli_seconds);
            ...
            // try {
            //     let resp = await fetcheEmailNotifications(2);
            //     for (let i = 0; i < resp.length; i++) {
